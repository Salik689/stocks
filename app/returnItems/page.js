...
export default function ReturnItemsPage() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setShoba('');
      setItems('');
      fetchReturnItems(); // Refresh return items list
    } catch (err) {
      ...
      <div style={{ padding: '2rem', maxWidth: '800px', margin: 'auto' }}>
        ...
        <form onSubmit={onSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <input
            ...
            placeholder="AIMS ID"
            value={aimsId}
            onChange={(e) => setAimsId(e.target.value)}
            required
          />
          <input
            type="text"
        ...
        <table style={{ marginTop: '2rem', width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {Array.isArray(allReturns) && allReturns.length > 0 ? (
              ...
              <tr>
                <td colSpan="4" style={{ padding: '1rem', textAlign: 'center' }}>No return items found.</td>
              </tr>
            )}
          </tbody>
        </table>
